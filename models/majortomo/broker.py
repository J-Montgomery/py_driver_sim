...
class Broker(Thread):
    ...
    def __init__(self, bind=DEFAULT_BIND_URL, heartbeat_interval=DEFAULT_HEARTBEAT_INTERVAL,
                 heartbeat_timeout=DEFAULT_HEARTBEAT_TIMEOUT,
                 busy_worker_timeout=DEFAULT_BUSY_WORKER_TIMEOUT,
                 zmq_context=None):
        super().__init__()
    ...
    def bind(self):
        ...
        """
        if self._socket:
            raise StateError("Socket is already bound")
    ...
    def handle_message(self, message):
        ...
        """
        if message.header == WORKER_HEADER:
            self._handle_worker_message(message)
        elif message.header == CLIENT_HEADER:
            self._handle_client_message(message)
        else:
            raise ProtocolError("Unexpected protocol header: {}".format(message.header.decode('utf8')))
    ...
    def _handle_worker_message(self, message):
        ...
        if message.command == HEARTBEAT:
            ...
            self._send_to_worker(message.client, DISCONNECT)
            raise ProtocolError("Unexpected command from worker: {}".format(message.command.decode('utf8')))
    ...
    def _handle_worker_heartbeat(self, worker_id):
        ...
        if worker is None:
            if not self._services.is_busy_worker(worker_id):
                ...
                self._send_to_worker(worker_id, DISCONNECT)
            return
    ...
    def _handle_worker_partial(self, worker_id, client_id, body):
        ...
        self._send_to_client(client_id, PARTIAL, body)
    ...
    def _handle_worker_final(self, worker_id, client_id, body):
        ...
        self._send_to_client(client_id, FINAL, body)
        now = time.time()
    ...
    def _handle_client_message(self, message):
        ...
        """
        assert message.command == REQUEST
        if len(message.message) < 2:
            raise ProtocolError("Client REQUEST message is expected to be at least 2 frames long, got {}".format(
                len(message.message)))
    ...
    def _dispatch_queued_messages(self):
        ...
        for message, worker, client in self._services.dequeue_pending():
            self._send_to_worker(worker.id, REQUEST, [client, b''] + message)
    ...
    def _purge_expired_workers(self):
        ...
        for worker in list(self._services.expired_workers()):  # Copying to list as we are going to mutate
            ...
            self._send_to_worker(worker.id, DISCONNECT)
    ...
    def _send_heartbeat(self):
        ...
        for worker in self._services.heartbeat_workers():
            ...
            self._send_to_worker(worker.id, HEARTBEAT)
    ...
    def _send_to_worker(self, worker_id, command, body=None):
        ...
        if body is None:
            body = []
        self._socket.send_multipart([worker_id, b'', WORKER_HEADER, command] + body)

    def _send_to_client(self, client_id, command, body):
        # type: (bytes, bytes, List[bytes]) -> None
        """Send message to client
        """
        self._socket.send_multipart([client_id, b'', CLIENT_HEADER, command] + body)

    @staticmethod
    def _parse_incoming_message(frames):
        # type: (List[bytes]) -> Message
        """Parse and verify incoming message
        """
        if len(frames) < 4:
            raise ProtocolError("Unexpected message length: expecting at least 4 frames, got {}".format(
                len(frames)))

        if frames[1] != b'':
            raise ProtocolError("Expecting empty frame 1, got {} bytes".format(len(frames[1])))

        return Message(client=frames[0], header=frames[2], command=frames[3], message=frames[4:])
...
class ServicesContainer:
    ...
    """
    def __init__(self, busy_workers_timeout=DEFAULT_BUSY_WORKER_TIMEOUT):
        # type: (float) -> None
    ...
    def add_worker(self, worker_id, service, expire_at, next_heartbeat):
        ...
        if worker_id in self._workers:
            raise StateError("Worker '{}' has already sent READY message".format(worker_id.decode('utf8')))
        worker = BrokerWorker(worker_id, service, expire_at, next_heartbeat)
    ...
    def set_worker_busy(self, worker_id, expire_at):
        ...
        if worker_id not in self._workers:
            raise StateError("Worker id {} is not in the list of available workers".format(id_to_int(worker_id)))
    ...
    def set_worker_available(self, worker_id, expire_at, next_heartbeat):
        ...
        if worker_id not in self._busy_workers:
            raise StateError("Worker id {} is not previously known or has expired".format(id_to_int(worker_id)))
    ...
    def remove_worker(self, worker_id):
        ...
        try:
            ...
            try:
                ...
                raise StateError("Worker id {} is not known".format(worker_id.decode('utf8')))
...
def _parse_args():
    ...
    parser.add_argument('-B', '--busy-worker-timeout', help='Timeout for busy workers (sec)', type=float,
                        default=DEFAULT_BUSY_WORKER_TIMEOUT)
